...
namespace WRRManagement.Infrastructure.ReservationRepository
{
    public class AvailablityRackRoomRepository : GenericRepository<AvailableRackRoom>, IAvailableRackRoom
    {
        ...
        private readonly ILogger<AvailablityRackRoomRepository> _logger;
        ...
        public AvailablityRackRoomRepository(WRRContext context, IRackRate rackRateRep, ITierLevel tierLevelRep, IHotelSystem hotelSystem, 
            ...
            IRoomAllocation roomAllocation, ILogger<AvailablityRackRoomRepository> logger) : base(context)
        {
        ...
        private List<RoomType> GetRoomsBaseOnGuestCount(int hotelId, int adults, int children)
        {
            ...
            {
                _logger.LogError(ex, "Error occurred while retrieving adult base rooms: {@adultBase}", new { hotelId, adults, children });
            }
        ...
        private List<RoomType> GetRoomsWithAllocation(List<RoomType> rooms, DateTime arrival, DateTime departure)
        {
            ...
            List<ParameterInfo> parameters = new List<ParameterInfo>();
            parameters.Add(P("Start", arrival));
            parameters.Add(P("End", departure));
            ...
            if (rooms.Count > 0)
            {
                ...
                {
                    bool valid = false;
                    parameters.Add(P("RoomID", room.RoomTypeID));
                    try
                    {
                    ...
                    {
                        _logger.LogError(ex, "Error occurred while checking room allocation: {@room}", new { room.RoomTypeID, arrival, departure });
                    }
